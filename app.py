...
timed = col2.toggle("Timed Mode")  # Changed to toggle button for better visibility
...
if st.session_state.quiz_active:
    ...
    if timed:
        ...
        st.progress((remaining / (num_questions * 120 if not 'Mock' in st.button else 7200)), text=f"Time Left: {mins:02d}:{secs:02d}")
        if remaining <= 0:
            st.session_state.quiz_active = False
            st.rerun()
    ...
    for i, q in enumerate(st.session_state.current_quiz):
        ...
        st.session_state.answers[i] = st.radio("Choose", q["options"], index=0 if st.session_state.answers[i] is None else q["options"].index(st.session_state.answers[i]), key=f"q{i}")
